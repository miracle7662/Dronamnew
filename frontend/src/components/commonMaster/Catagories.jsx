...
import React, { useState, useEffect, useMemo } from 'react';
import { Container, Row, Col, Card, Table, Button, Modal, Form, Alert, Badge, InputGroup, Pagination, Stack } from 'react-bootstrap';
    ...
    try {
      ...
      if (editingCategory) {
        ...
        setCategories(prev => prev.map(category =>
          category.categories_id === editingCategory.categories_id ? { ...category, ...formData, status: formData.status } : category
        ));
      } else {
        const response = await axios.post('http://localhost:3001/api/categories', {
          ...formData,
  ...
  return (
    <Container fluid className="py-4">
      ...
      <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)}>
        ...
        <Modal.Body>
          Are you sure you want to delete <strong>{categoryToDelete?.categories_name}</strong>? This action cannot be undone.
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowDeleteModal(false)}>
            Cancel
