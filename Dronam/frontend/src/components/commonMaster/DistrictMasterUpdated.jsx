...
const DistrictMasterUpdated = () => {
  ...
  const loadDistricts = async () => {
    ...
    setError('');
    try {
      const response = await axios.get('http://localhost:3001/api/districts/updated');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDistrict) {
        await axios.put(`http://localhost:3001/api/districts/updated/${editingDistrict.district_id}`, {
          ...formData,
          updated_by_id: userId
        });
        await loadDistricts();
      } else {
        const response = await axios.post('http://localhost:3001/api/districts/updated', {
          ...formData,
  ...
  const confirmDelete = async () => {
    if (districtToDelete) {
      try {
        await axios.delete(`http://localhost:3001/api/districts/updated/${districtToDelete.district_id}`);
        await loadDistricts();
...
export default DistrictMasterUpdated;
