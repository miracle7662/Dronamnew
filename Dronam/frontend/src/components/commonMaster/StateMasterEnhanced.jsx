...
const StateMasterEnhanced = () => {
  ...
  const loadStates = async () => {
    ...
    setError('');
    try {
      const response = await axios.get('http://localhost:3001/api/states/enhanced');
  ...
  const validateForm = () => {
    ...
    if (!formData.state_code.trim()) {
      ...
      newErrors.state_code = 'State code must be 3 characters or less';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingState) {
        await axios.put(`http://localhost:3001/api/states/enhanced/${editingState.state_id}`, {
          ...formData,
          updated_by_id: userId
        });
        await loadStates();
      } else {
        const response = await axios.post('http://localhost:3001/api/states/enhanced', {
          ...formData,
  ...
  const confirmDelete = async () => {
    if (stateToDelete) {
      try {
        await axios.delete(`http://localhost:3001/api/states/enhanced/${stateToDelete.state_id}`);
        await loadStates();
  ...
  const filteredStates = states.filter(state =>
    ...
    (state.description && state.description.toLowerCase().includes(searchTerm.toLowerCase())) ||
                    ...
                    {currentStates.map((state, index) => (
                      <tr key={state.state_id}>
                        ...
                        <td>
                          {state.country_name ? (
                            <div className="d-flex align-items-center">
                              <Globe size={14} className="text-muted me-1" />
                              {state.country_name}
                            </div>
                          ) : (
                            <span className="text-muted">-</span>
                        ...
                        <td>
                          {state.description ? (
                            <span className="text-muted">{state.description}</span>
                          ) : (
                            <span className="text-muted">-</span>
                          )}
                        </td>
...
export default StateMasterEnhanced;
